function Plan = generate_Plan_Heuristic3(ws, initial_locations,...
    ...
    if ~isempty(badSessions)
        for i = 1:length(badSessions)
            ...
            if badSession.agentId == n
                for m = 1:badSession.length-1
                    nowCell = ws.n2r(badSession.ascCells(m));
                    nextCell = ws.n2r(badSession.ascCells(m+1));
                    weighted_adjr_n(nowCell, nextCell) =...
                        weighted_adjr_n(nowCell, nextCell) +...
                        optimize_weight*badSession.ascBottleNumber(m)*m*adjr_n(nowCell, nextCell);
                end
            end
        end
    end
