        ...
        for n = 1:N
           ...
           if ~dummy_runs_completed(n)
            ...
            if ~runs_completed(n)
                ...
                agents(n).globalTime = agents(n).globalTime + 1;
                next_state_n = agents(n).path(agents(n).localTime+1);
                % is the next state conflicting
%                 min_time_index = agents(n).localTime+1;
                other_agents_cleared = 1;
                for m = 1:N
                    if m ==n
                        continue
                    end
                    if ~isempty(find(agents(m).path == next_state_n, 1))
                        min_time_index_m = find(agents(m).path == next_state_n);
                        if (min_time_index_m <  (agents(n).localTime+1)) & (agents(m).localTime <= min_time_index_m)
                            other_agents_cleared = 0;
                        end
                    end
                end
                % if conflicting
                if other_agents_cleared && rand(1) < prob_succ(n)
                    agents(n).localTime = agents(n).localTime + 1;
                    agents(n).position = agents(n).path(agents(n).localTime);
                else
                    total_messsages = total_messsages + 1;
                    continue
                end
                                    
                if agents(n).localTime == length(agents(n).path)
                    runs_completed(n) = 1;
